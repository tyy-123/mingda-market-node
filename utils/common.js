...
async function getUser(req, res) {
    ...
    let result = await handleDB(res,'users','find','查询数据库错误',`account = '${account}' && password = '${password}'`)
    return result
}
...
async function getUserExist(req, res) {
    ...
    let result = await handleDB(res, 'users', 'find', '查询数据库错误', `account='${email}'`)
    return result
}
...
async function getUserInfo(req, res) {
    ...
    return await handleDB(res, 'users', 'find', '查询数据库错误', `c_id='${c_id}'`);
}
...
async function addOrder(req, res) {
    ...
    //更新发单者信息
    handleDB(res, 'users', 'sql', '更新数据库错误', 
...
async function getReleased(req, res) {
    ...
    return await handleDB(res, 'orders', 'find', '查询数据库错误', `oc_id='${oc_id}'`)
}

//接单
async function handleOrder(req, res) {
    const {c_freeze_fund, c_expendable_fund, total_fund, c_id, o_id} = req.query
    const freeze_fund = c_freeze_fund * 1 + total_fund * 1
    const expendable_fund = c_expendable_fund - total_fund
    await handleDB(res, 'users', 'sql', '更新数据库错误', 
    `update users set freeze_fund='${freeze_fund}', expendable_fund='${expendable_fund}' where c_id='${c_id}'`
    )
    await handleDB(res, 'orders', 'sql', '更新数据库错误',
...
async function catcher(req, res) {
    const {c_id} = req.query
    const result = await handleDB(res, 'orders', 'find', '查询数据库错误', `c_id='${c_id}'`)
    return result
}
...
async function confirm(req, res) {
    ...
    const result = await handleDB(res, 'orders', 'sql','更新数据库错误',
...
async function confirmSingle(req, res) {
    const {o_id, c_id, efficiency_margin, safety_margin, commission} = req.query
...
async function getAttention(req, res) {
    ...
    const result = await handleDB(res, 'attentions', 'find', '查询数据库错误', `a_id='${a_id}'`)
    return result
}
...
async function amendPass(req, res) {
    ...
    const result = await handleDB(res, 'users', 'sql', '更新数据库错误', 
...
async function getAtt(req, res) {
    const {a_id} = req.query
    const result = await handleDB(res, 'attentions', 'sql', '查询数据库错误', 
...
async function query(req, res) {
    ...
    const result = await handleDB(res, 'orders', 'sql', '查询数据库错误',
...
async function addAttention(req, res) {
    ...
    const result = await handleDB(res, 'attentions', 'find', '查询数据库错误', `a_id=${a_id}&&ad_id=${ad_id}`)
    if(result.length) {
        return false
    }
    await handleDB(res, 'attentions', 'insert', '插入数据库错误', {a_id, ad_id});  
    await handleDB(res, 'users', 'sql', '更新数据库错误', 
...
async function cancelAtt(req, res) {
    const {a_id, ad_id} = req.query
    const result = await handleDB(res, 'attentions', 'delete', '删除数据库错误', `a_id=${a_id}&&ad_id=${ad_id}`)
    await handleDB(res, 'users', 'sql', '更新数据库错误', 
            `update users set attention='attention-1' where 'c_id=${a_id}'`
    )  
    await handleDB(res, 'users', 'sql', '更新数据库错误',
